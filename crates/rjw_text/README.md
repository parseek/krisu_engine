...
- `create_buffer`：cosmic-text 排版（Metrics / Attrs / Shaping / Align），内部按输入做 **LRU 排版缓存**（上限 [`MAX_LAYOUT_CACHE`]=128），相同输入直接克隆跳过重复整形。
...
- `create_buffer`: cosmic-text layout (Metrics / Attrs / Shaping / Align), with an internal **LRU layout cache** (cap [`MAX_LAYOUT_CACHE`]=128) so repeated inputs clone the shaped `Buffer` instead of re-shaping.
